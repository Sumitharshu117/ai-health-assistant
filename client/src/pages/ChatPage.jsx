...
export default function ChatPage() {
  ...
  const { messages, loading, activeDiagnosis } = useSelector(
    (state) => state.chat
  );
  ...
  const handleImageUpload = async (file) => {
    ...
    try {
      ...
      dispatch(
        sendMessageFailure("Failed to process the image. Please try again.")
      );
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        const city = window.prompt(
          "Unable to get your location. Please enter your city (e.g. Mumbai, Delhi):"
        );

        if (city) {
          const cityUrl = `https://www.google.com/maps/search/hospitals+in+${encodeURIComponent(
            city
          )}`;
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="hidden lg:block w-1/4 px-4 pr-8 pt-6">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-lg space-y-3">
            <h2 className="text-indigo-600 dark:text-indigo-400 text-lg font-semibold">
              Health Tips 💡
            </h2>
